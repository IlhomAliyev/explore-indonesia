...
export default {
  accordion() {
    ...
    for (let eachArticle of allArticles) {
      ...
      const exactParagraphShortText =
        exactParagraph.innerHTML.slice(0, 21) + '...';
      ...
      function openAccordion(event) {
        if (!eachArticle.classList.contains('_active')) {
          ...
          eachArticle.classList.contains('_active') &&
          !event.target.closest('.sub-article-video-wrapper')
        ) {
    ...
    const subArticleDescription = document.querySelector(
      '.sub-article-video-description'
    );
    const subParagraph = document.querySelector(
      '.sub-article-video__paragraph'
    );
    ...
    subArticleShowMore.addEventListener('click', openSubArticle, {
      capture: true,
    });
